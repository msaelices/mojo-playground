...
fn demo_nbody() raises:
    ...
    with DeviceContext() as ctx:
        ...
        var pos_x_host = ctx.enqueue_create_host_buffer[DType.float32](
            NUM_PARTICLES
        )
        var pos_y_host = ctx.enqueue_create_host_buffer[DType.float32](
            NUM_PARTICLES
        )
        var pos_z_host = ctx.enqueue_create_host_buffer[DType.float32](
            NUM_PARTICLES
        )
        var vel_x_host = ctx.enqueue_create_host_buffer[DType.float32](
            NUM_PARTICLES
        )
        var vel_y_host = ctx.enqueue_create_host_buffer[DType.float32](
            NUM_PARTICLES
        )
        var vel_z_host = ctx.enqueue_create_host_buffer[DType.float32](
            NUM_PARTICLES
        )
        ...
        for step in range(NUM_ITERATIONS):
            ...
            ctx.enqueue_function_checked[update_particles_kernel](
                pos_x,
                pos_y,
                pos_z,
                vel_x,
