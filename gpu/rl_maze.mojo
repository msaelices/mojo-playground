...
fn get_valid_actions_kernel(
    ...
    if row > 0:
        ...
        valid_actions[state_idx, UP] = 1 - maze[next_state][0]  # Valid if not a wall (0 or 2)
    else:
...
fn monte_carlo_episode_kernel(
    ...
    if steps > 0:
        ...
        for t in range(steps - 1, -1, -1):  # Reverse order
            ...
            var learning_rate = Float32(1.0 / (1.0 + 0.01 * Float32(thread_id % 10)))  # Vary learning rate
            q_table[state_idx, action_idx] = current_q + learning_rate * (returns - current_q)
...
fn main() raises:
    ...
    with DeviceContext() as ctx:
        ...
        var maze_buffer = ctx.enqueue_create_host_buffer[DType.int32](NUM_STATES)
        var valid_actions_buffer = ctx.enqueue_create_host_buffer[DType.int32](NUM_STATES * NUM_ACTIONS)
        var q_table_buffer = ctx.enqueue_create_host_buffer[DType.float32](NUM_STATES * NUM_ACTIONS)
        var episode_seeds_buffer = ctx.enqueue_create_host_buffer[DType.int32](NUM_BLOCKS * NUM_THREADS)
        ...
        var valid_actions_dev = ctx.enqueue_create_buffer[DType.int32](NUM_STATES * NUM_ACTIONS)
        var q_table_dev = ctx.enqueue_create_buffer[DType.float32](NUM_STATES * NUM_ACTIONS)
        var episode_seeds_dev = ctx.enqueue_create_buffer[DType.int32](NUM_BLOCKS * NUM_THREADS)
        ...
        for episode in range(1, EPISODES + 1, NUM_THREADS * NUM_BLOCKS):
            ...
            ctx.enqueue_function[monte_carlo_episode_kernel](
                maze, valid_actions, q_table, episode_seeds,
                grid_dim=NUM_BLOCKS,
        ...
        var optimal_path_buffer = ctx.enqueue_create_host_buffer[DType.int32](MAX_STEPS)
        ...
        var path_length = LayoutTensor[DType.int32, Layout.row_major(1)](path_length_dev)
        var optimal_path = LayoutTensor[DType.int32, Layout.row_major(MAX_STEPS)](optimal_path_dev)
        
        ctx.enqueue_function[find_optimal_path_kernel](
            maze, valid_actions, q_table, path_length, optimal_path,
            grid_dim=1,
            block_dim=1
        )
        ...
        for i in range(path_len):
            ...
            if maze_with_path[state] == 0:  # Only mark empty spaces, not start/goal
