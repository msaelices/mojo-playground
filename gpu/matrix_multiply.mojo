...
fn matrix_multiply_shared_kernel(
    A: MatrixA, B: MatrixB, C: MatrixC
):
...
fn verify_result(host_a: DeviceBuffer[DType.float32], host_b: DeviceBuffer[DType.float32], host_c: DeviceBuffer[DType.float32]) -> Bool:
    ...
    for i in range(M):
        for j in range(N):
            ...
            if diff < -0.01 or diff > 0.01:
                print("Error at [", i, ",", j, "]: Got", C[i, j][0], "Expected", expected)
                correct = False
