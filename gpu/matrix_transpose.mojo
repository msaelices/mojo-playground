...
fn naive_transpose_kernel(
    input: InputMatrix, output: OutputMatrix
):
    ...
    for i in range(M):
        for j in range(N):
            if input[i, j][0] != output[j, i][0]:
                print(
                    "Error at [", i, ",", j, "]: Input =", 
                    input[i, j][0], "Output =", output[j, i][0]
                )
                correct = False
...
fn main() raises:
    with DeviceContext() as ctx:
        ...
        var grid_dim = ((N + BLOCK_SIZE - 1) // BLOCK_SIZE, (M + BLOCK_SIZE - 1) // BLOCK_SIZE)
